...
$conn = $dbh->prepare("SELECT * FROM cvbuilder WHERE userId = ?");
$conn->execute([$userId]);
$fetch = $conn->fetchAll(PDO::FETCH_ASSOC);

$stmt = $dbh->prepare("SELECT expTitle, expCompany, expMonths, expDescription FROM cvbuilder WHERE userId = ?");
$stmt->execute([$userId]);
$expData = $stmt->fetchAll(PDO::FETCH_ASSOC);
  ...
  <?php foreach ($fetch as $cvData) { ?>
    <div class="container">
      <div class="header">
        ...
        <div class="about">
          <span class="desc">
            <?php echo nl2br($cvData['generalDescription']) ?>
          </span>
        </div>
      </div>
      <div class="details">
                ...
                for ($i = 0; $i < count($titles); $i++) {
                  ...
                  <div class="section__list-item">
                    ...
                    <div class="right">
                      ...
                      <div class="desc"><?php echo nl2br($descriptions[$i]); ?></div>
                    </div>
                  </div>
                  <?php
                }
