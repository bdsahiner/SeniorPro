...
$conn = $dbh->prepare("DELETE FROM bookmarks WHERE jobId = ? AND employeeId = ?");
if ($conn->execute([$jobId, $userId])) {
    $log->add($userFullName['fullName'] . ' removed ' . $jobName['jobTitle'] . ' from bookmark from the IP address ' . $ipAddress . '.');
=======
$jobId = $_GET['id'];
$userId = $_SESSION['id'];
